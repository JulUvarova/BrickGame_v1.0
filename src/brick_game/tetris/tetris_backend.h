...
/*!
\ingroup status
Инициализирует структуру Game_t, создает игровое поле, устанавливает начальные
значения игры Срабатывает при статусе START Устанавливает состояние SPAWN
...
/*!
\ingroup status
Создает матрицы текущего и следующего блоков, заполняет их рандомной фигурой,
устанавливает параметры блоков Срабатывает при статусе SPAWN При заполнении
первой строки игрового поля переводит состояние на GAMEOVER, иначе - ACTION
*/
void block_spawn();

/*!
\ingroup status
Вызыает функции обработки движения в зависимости от выбранного пользователем
действия \param[in] action Действие пользователя: вправо, влево, вниз, падение,
ротация, простой Срабатывает при статусе ACTION
*/
...
/*!
\ingroup move
Считает разницу между start_time и текущем временем, сравнивает ее сщ значением
speed Если разница больше - переход в статус SHIFTING Срабатывает при состоянии
ACTION
*/
void check_time();
...
В случае невыделения памяти происходит очистка частично выделенного и
возвращение NULL
*/
...
/*!
\ingroup matrix
Удаляет из поля field указанную строку. Вышележащие строки точки смещаются на
индекс ниже \param[in] row Индекс сроки поля field
*/
void delete_row(int row);

/*!
\ingroup matrix
Выделяет память по навую матрицу размера size и копирует в нее данные из матрицы
src \param[in] src Указатель на матрицу, которую нужно скопировать \param[in]
size Размер скопированной матрицы
*/
